...
public interface ShipperRepository extends JpaRepository<Shipper, Long> {
    @Query(
        value = "SELECT * FROM shippers WHERE CONCAT(fullname, ' ', dia_chi,' ', sdt, ' ',email, ' ', cccd) LIKE %?1% ORDER BY id DESC LIMIT ?2,?3",
        nativeQuery = true)
    ...
    @Query(
        value="SELECT count(id) FROM shippers WHERE CONCAT(fullname, ' ', dia_chi,' ', sdt, ' ',email, ' ', cccd) LIKE %?1%",
        nativeQuery=true)
